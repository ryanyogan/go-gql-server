...
import (
	...
	"yogan.dev/go-gql-server/internal/gql/models"
	tf "yogan.dev/go-gql-server/internal/gql/resolvers/transformations"
	dbm "yogan.dev/go-gql-server/internal/orm/models"
)
...
func userCreateUpdate(r *mutationResolver, input models.UserInput, update bool, ids ...string) (*models.User, error) {
	...
	if !update {
		db = db.Create(dbo).First(dbo) // Create a user
	} else {
		db = db.Model(&dbo).Update(dbo).First(dbo) // Update it
	}
